...
async def control_panel(client_instance, message: Message):
    logger.info(
        "[CONTROL] Получено сообщение: chat_id=%s, from_user_id=%s, text='%s'",
        message.chat.id,
    ...
    if not message.from_user or message.chat.id != message.from_user.id:
        logger.info("[CONTROL] Не Saved Messages, пропускаем")
        return
    if not message.text:
        logger.info("[CONTROL] Нет текста, пропускаем")
        return

    cmd, args = parse_control_command(message.text)
    if not cmd:
        logger.info("[CONTROL] Не команда, пропускаем")
        return

    logger.info("[CONTROL] Выполняем команду: %s с аргументами: %s", cmd, args)
